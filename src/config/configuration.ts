...
export const getConfiguration = () =>
    ({
        ...
        database: {
            ...
            //synchronize: true, // Disable sync model & database
            synchronize: false,
            logging: ['error'],
            //logging: ['query', 'error'],
        ...
        databaseECharge: {
            type: 'mysql',
            host: process.env.MYSQL_ECHARGE_HOST,
            port: Number.parseInt(process.env.MYSQL_ECHARGE_PORT, 10),
            username: process.env.MYSQL_ECHARGE_USERNAME,
            password:
                process.env.MYSQL_ECHARGE_PASSWORD ||
                process.env.MYSQL_ECHARGE_ROOT_PASSWORD ||
                '',
            database: process.env.MYSQL_ECHARGE_DATABASE,
            entities: [__dirname + '/../**/entities/*.entity.{ts,js}'],
            migrations: ['dist/src/migrations/**/*.js'],
            autoLoadEntities: true,
            /** https://typeorm.io/migrations */
        ...
        // steve OCPP
        steveServer: {
            apiEndpoint: process.env.STEVE_API_ENDPOINT,
